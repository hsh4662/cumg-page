...
<html xmlns="http://www.w3.org/1999/xhtml" lang="<?php @print(Context::getLangType());?>" xml:lang="<?php @print(Context::getLangType());?>">
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<meta name="viewport" content="width=device-width, initial-scale=1, minimum-scale=1" />
<title><?php @print(Context::getBrowserTitle());?></title>
<?php @$__Context->css_files = Context::getCssFile();?>
<?php $__Context->Context->__idx[0]=0;if(count($__Context->css_files))  foreach($__Context->css_files as $__Context->key => $__Context->css_file){$__Context->__idx[1]=($__Context->__idx[1]+1)%2; $__Context->cycle_idx = $__Context->__idx[1]+1; ?>
...
<?php $__Context->Context->__idx[1]=0;if(count($__Context->js_files))  foreach($__Context->js_files as $__Context->key => $__Context->js_file){$__Context->__idx[2]=($__Context->__idx[2]+1)%2; $__Context->cycle_idx = $__Context->__idx[2]+1; ?>
...
<?php $__Context->Context->__idx[2]=0;if(count($__Context->js_body_files))  foreach($__Context->js_body_files as $__Context->key => $__Context->js_file){$__Context->__idx[3]=($__Context->__idx[3]+1)%2; $__Context->cycle_idx = $__Context->__idx[3]+1; ?>
<?php  if($__Context->js_file['targetie']){ ?>
    <!--[if <?php @print($__Context->js_file['targetie']);?>]>
<?php  } ?>
    <script type="text/javascript" src="<?php @print($__Context->js_file['file']);?>"></script>
