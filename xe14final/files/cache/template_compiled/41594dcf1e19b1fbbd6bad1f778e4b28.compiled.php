...
<?php  if($__Context->grant->manager){ ?>
            <th scope="col"><input type="checkbox" onclick="XE.checkboxToggleAll({ doClick:true }); return false;" title="Check All" /></th>
<?php  } ?>
          </tr>
        </thead>
        <tbody>
<?php  if(!$__Context->document_list && !$__Context->notice_list){ ?>
          <tr>
            <td colspan="<?php  if($__Context->grant->manager){ ?><?php @print(count($__Context->list_config)+1);?><?php  }else{ ?><?php @print(count($__Context->list_config));?><?php  } ?>">
...
<?php  if(count($__Context->document_list)){ ?><?php @$__Context->_height = count($__Context->list_config)*18;?>
<?php  if($__Context->list_config['thumbnail']){ ?><?php @$__Context->_height += $__Context->module_info->thumbnail_height;?><?php  } ?>
<?php  if($__Context->list_config['summary']){ ?><?php @$__Context->_height += 60;?><?php  } ?>
<?php  if($__Context->list_config['last_post']){ ?><?php @$__Context->_height += 30;?><?php  } ?>
...
<?php  if(!$__Context->document->isNotice()){ ?>
        ...
        <li style="width:<?php @print($__Context->module_info->thumbnail_width+18);?>px;height:<?php @print($__Context->_height);?>px;"><?php @$__Context->_meta_display = false;?>
...
<?php  if($__Context->val->idx == -1){ ?>
<?php  if($__Context->val->type == 'no'){ ?>
<?php  if($__Context->_meta_display){ ?>
      </ul><?php @$__Context->_meta_display = false;?>
<?php  } ?>
          <p class="num"><?php  if($__Context->document_srl == $__Context->document->document_srl){ ?>&raquo;<?php  }else{ ?><?php @print($__Context->no);?><?php  } ?></p>
...
<?php  } ?>
          ...
          <div class="title">
<?php  if($__Context->grant->manager){ ?>
            <input type="checkbox" name="cart" value="<?php @print($__Context->document->document_srl);?>" onclick="doAddDocumentCart(this)" <?php  if($__Context->document->isCarted()){ ?>checked="checked"<?php  } ?> /> 
<?php  } ?>
            <a href="<?php @print(getUrl('document_srl',$__Context->document->document_srl, 'listStyle', $__Context->listStyle, 'cpage',''));?>"><?php @print($__Context->document->getTitle($__Context->module_info->subject_cut_size));?></a>
            <?php @print($__Context->document->printExtraImages(60*60*$__Context->module_info->duration_new));?>	
<?php  if($__Context->document->getCommentCount()){ ?>
            <span class="replyNum" title="Replies">[<?php @print($__Context->document->getCommentCount());?>]</span>
<?php  } ?>
<?php  if($__Context->document->getTrackbackCount()){ ?>
            <span class="trackbackNum" title="Trackbacks">[<?php @print($__Context->document->getTrackbackCount());?>]</span>
<?php  } ?>
          </div>
...
<?php  } ?>
          <p class="summary"><a href="<?php @print(getUrl('document_srl',$__Context->document->document_srl, 'listStyle', $__Context->listStyle, 'cpage',''));?>"><?php @print($__Context->document->getSummary(50));?></a></p>
...
<?php  }else{ ?>
<?php  if(!$__Context->_meta_display){ ?>
          <ul class="meta"><?php @$__Context->_meta_display = true;?>
<?php  } ?>
